...
# ---------------------------------------------------------------------------
# _extract_ac_commands
# ---------------------------------------------------------------------------

class TestExtractAcCommands:
    def _make_executor(self, tmp_project, step_content):
        d = tmp_project / "phases" / "test-phase"
        d.mkdir(exist_ok=True)
        index = {"project": "T", "phase": "test", "steps": [{"step": 0, "name": "s", "status": "pending"}]}
        (d / "index.json").write_text(json.dumps(index))
        (d / "step0.md").write_text(step_content)
        inst = ex.StepExecutor.__new__(ex.StepExecutor)
        inst._phase_dir = d
        inst._root = str(tmp_project)
        return inst

    def test_extracts_commands(self, tmp_project):
        content = "# Step 0\n\n## Acceptance Criteria\n\n```bash\npnpm build\npnpm test\n```\n"
        inst = self._make_executor(tmp_project, content)
        assert inst._extract_ac_commands(0) == ["pnpm build", "pnpm test"]

    def test_strips_inline_comments(self, tmp_project):
        content = "# Step 0\n\n## Acceptance Criteria\n\n```bash\npnpm build   # 에러 없음\n```\n"
        inst = self._make_executor(tmp_project, content)
        assert inst._extract_ac_commands(0) == ["pnpm build"]

    def test_skips_unfilled_placeholders(self, tmp_project):
        content = "# Step 0\n\n## Acceptance Criteria\n\n```bash\n{빌드 커맨드}\n{테스트 커맨드}\n```\n"
        inst = self._make_executor(tmp_project, content)
        assert inst._extract_ac_commands(0) == []

    def test_no_ac_section_returns_empty(self, tmp_project):
        content = "# Step 0\n\n## 작업\n\n무언가 한다.\n"
        inst = self._make_executor(tmp_project, content)
        assert inst._extract_ac_commands(0) == []

    def test_missing_step_file_returns_empty(self, tmp_project):
        d = tmp_project / "phases" / "test-phase2"
        d.mkdir(exist_ok=True)
        inst = ex.StepExecutor.__new__(ex.StepExecutor)
        inst._phase_dir = d
        assert inst._extract_ac_commands(99) == []
...
class TestVerifyAc:
    def _make_executor_with_step(self, tmp_project, step_content):
        d = tmp_project / "phases" / "verify-phase"
        d.mkdir(exist_ok=True)
        index = {"project": "T", "phase": "verify", "steps": [{"step": 0, "name": "s", "status": "pending"}]}
        (d / "index.json").write_text(json.dumps(index))
        (d / "step0.md").write_text(step_content)
        inst = ex.StepExecutor.__new__(ex.StepExecutor)
        inst._phase_dir = d
    ...
    def test_passes_when_command_succeeds(self, tmp_project):
        content = "## Acceptance Criteria\n\n```bash\ntrue\n```\n"
        inst = self._make_executor_with_step(tmp_project, content)
        passed, msg = inst._verify_ac(0)
        assert passed is True
        assert msg == ""

    def test_fails_when_command_fails(self, tmp_project):
        content = "## Acceptance Criteria\n\n```bash\nfalse\n```\n"
        inst = self._make_executor_with_step(tmp_project, content)
        passed, msg = inst._verify_ac(0)
    ...
    def test_passes_when_no_ac_section(self, tmp_project):
        content = "# Step\n\n## 작업\n\n무언가.\n"
        inst = self._make_executor_with_step(tmp_project, content)
        passed, msg = inst._verify_ac(0)
        assert passed is True

    def test_error_message_includes_command_output(self, tmp_project):
